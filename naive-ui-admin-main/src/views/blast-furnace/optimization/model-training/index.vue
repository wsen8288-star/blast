<template>
  <div
    class="model-training min-h-[calc(100vh-80px)] w-full flex-1 p-4 md:p-6 overflow-auto box-border"
  >
    <n-card title="模型训练" class="mb-4 training-main-card">
      <!-- 训练配置 -->
      <n-form label-placement="left" label-width="120" class="mb-4 training-form">
      ...
      <n-tabs type="line" v-model:value="activeTab">
        <n-tab-pane name="process" tab="训练过程" display-directive="show">
          <div class="training-panel">
            <n-card title="训练状态" size="small" class="mb-4 training-sub-card">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
              <n-card title="训练过程损失 (trainLoss)" size="small" class="training-sub-card">
                <div ref="lossChartRef" style="height: 300px"></div>
              </n-card>
              <n-card title="模型精度 (R²)" size="small" class="training-sub-card">
                <div ref="accuracyChartRef" style="height: 300px"></div>
              </n-card>
            </div>

            <div class="grid grid-cols-3 gap-4">
              <n-card title="实时指标" size="small" class="training-sub-card">
                <div class="grid grid-cols-2 gap-4">
                  <n-statistic
              ...
              <n-card title="训练日志" size="small" class="col-span-2 training-sub-card">
          ...
          <div class="training-panel">
            <div class="mb-4 flex justify-end">
              <n-popconfirm @positive-click="handleBatchDelete">
                <template #trigger>
                  <n-button round type="error" :disabled="checkedRowKeys.length === 0">
                    批量删除
                  </n-button>
                </template>
    ...
    >
      <n-descriptions bordered :column="1" v-if="currentDetail">
        ...
        <n-descriptions-item label="训练参数">
          <div class="text-xs">
            <!-- 显示模型配置参数 -->
            <div v-if="currentDetail.modelConfig">
              <div
                v-for="(value, key) in formatConfig(
                  currentDetail.modelConfig,
                  currentDetail.modelType
                )"
                :key="key"
                class="mb-1"
              >
            ...
            <!-- 显示训练通用参数 (如GPR的迭代次数/批次) -->
            <div>
              <div
                v-for="(value, key) in formatTrainingParams(currentDetail)"
                :key="key"
                class="mb-1"
              >
                <span class="font-bold">{{ key }}:</span> {{ value }}
...
<script setup lang="ts">
  ...
  // 格式化训练参数信息（从ModelTraining表）
  const formatTrainingParams = (row: any) => {
    const result: Record<string, any> = {};
    if (!row) return result;

    const type = row.modelType;

    // 高斯过程回归模型需要显示迭代次数和批次大小
    if (type === 'gpr') {
...
<style lang="less" scoped>
  ...
  .training-form :deep(.n-base-selection),
  .training-form :deep(.n-input),
  .training-form :deep(.n-input-number) {
    border-radius: 10px;
  }
</style>
