<template>
  <div
    class="role-management min-h-[calc(100vh-80px)] w-full flex-1 p-4 md:p-6 overflow-hidden box-border space-y-4"
  >
    <n-card :bordered="false" size="small" class="role-main-card">
      <div class="flex items-start justify-between">
        <div>
          <div class="text-base font-semibold role-title">角色管理</div>
          <div class="mt-1 text-xs role-subtitle">维护角色、分配权限并联动前后端鉴权</div>
        </div>
        <div class="flex items-center gap-2">
          ...
          <n-button round v-permission="['system:role:write']" type="primary" @click="openCreate">
            <template #icon>
    ...
    <n-card :bordered="false" size="small" class="role-main-card">
      <div class="flex flex-wrap items-center gap-3">
        ...
        />
        <div class="flex items-center gap-2">
          <n-button round size="small" type="primary" :loading="loading" @click="applySearch">查询</n-button>
          <n-button round size="small" @click="resetSearch">重置</n-button>
        </div>
      </div>
      ...
      <div class="mt-3 flex items-center justify-between">
        <div class="text-xs role-subtitle">
          共 {{ roleList.length }} 条
        </div>
        <div class="flex items-center gap-2">
    ...
    <n-modal
      ...
      preset="card"
      title="分配权限"
      class="w-[720px] role-modal-card"
    >
      <div class="space-y-3">
        <div class="flex items-center justify-between">
          <div class="text-sm role-subtitle">角色：{{ currentRole?.roleName }} ({{ currentRole?.roleCode }})</div>
          <n-button round size="small" secondary :loading="permissionLoading" @click="loadPermissionData">刷新权限</n-button>
        </div>
        <n-tree
          checkable
          block-line
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <n-button round @click="showPermissionDialog = false">取消</n-button>
          <n-button round v-permission="['system:role:write']" type="primary" :loading="permissionSaving" @click="saveRolePermissions">
            确定
          </n-button>
        </div>
      </template>
...
<script setup lang="ts">
  ...
  async function batchDelete() {
    ...
    try {
      await Promise.all(ids.map((id) => systemApi.role.delete(id)));
...
<style scoped>
  ...
  .role-management :deep(.n-base-selection),
  .role-management :deep(.n-input),
  .role-management :deep(.n-tree) {
    border-radius: 10px;
  }

  .role-management :deep(.n-drawer-content),
  .role-modal-card {
    border-radius: 12px;
    border: 1px solid var(--n-border-color);
