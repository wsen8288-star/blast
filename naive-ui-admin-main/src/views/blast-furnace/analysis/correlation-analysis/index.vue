<template>
  <div class="correlation-analysis min-h-[calc(100vh-80px)] w-full flex-1 p-4 md:p-6 overflow-hidden box-border">
    <n-card title="参数相关性分析" class="mb-4 correlation-main-card" :bordered="false">
      <n-form label-placement="left" label-width="120" class="mb-4 correlation-form">
        ...
        <div class="flex justify-end">
          <n-button round type="primary" @click="startAnalysis" :loading="loading">开始分析</n-button>
          <n-button round @click="resetAnalysis" class="ml-2" :disabled="loading">重置</n-button>
        </div>
      </n-form>

      <n-tabs type="line" v-model:value="activeTab">
        <n-tab-pane name="matrix" tab="相关性矩阵">
          <div class="correlation-panel transition-colors duration-300">
            <n-card
              title="相关性系数矩阵"
              size="small"
              class="mb-4"
            ...
            <n-card title="相关性热力图" size="small" :bordered="false" class="correlation-sub-card">
        ...
        <n-tab-pane name="scatter" tab="散点图分析">
          <div class="correlation-panel transition-colors duration-300">
            <n-form label-placement="left" label-width="120" class="mb-4 correlation-form">
              ...
              <div class="flex justify-end">
                <n-button round type="primary" @click="updateScatterPlot">更新散点图</n-button>
              </div>
            </n-form>

            <n-card title="参数散点图" size="small" class="correlation-sub-card">
              <div class="h-80">
            ...
            <n-card title="分析报告" size="small" class="mb-4 result-card">
              <div class="p-4">
                ...
                <div v-else-if="currentReport">
                  ...
                  <div class="correlation-inner-panel transition-colors duration-300">
                    <n-text depth="3">
                      <strong>分析建议：</strong>
                    </n-text>
...
<script setup lang="ts">
  ...
  const dateRange = ref([new Date('2026-01-01'), new Date()]);
  ...
  const resetAnalysis = () => {
    ...
    dateRange.value = [new Date('2026-01-01'), new Date()];
...
/* 所有强行覆盖 Naive UI 的穿透样式已被清理干净 */
  ...
  .correlation-sub-card {
    border: 1px solid var(--n-border-color);
    border-radius: 10px;
    box-shadow: none;
  }

  .correlation-inner-panel {
    padding: 14px;
    border-radius: 10px;
    border: 1px solid var(--n-border-color);
    background-color: var(--n-color);
  ...
  .correlation-form :deep(.n-base-selection),
  .correlation-form :deep(.n-input),
  .correlation-form :deep(.n-input-number),
  .correlation-form :deep(.n-date-picker) {
    border-radius: 10px;
  }
