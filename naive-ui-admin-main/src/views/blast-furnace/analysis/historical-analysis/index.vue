<template>
  <div
    class="historical-analysis min-h-[calc(100vh-80px)] w-full flex-1 p-4 md:p-6 overflow-auto box-border"
  >
    <n-card title="历史数据分析" class="mb-4 historical-main-card">
      <!-- 查询条件 -->
      <n-form label-placement="left" label-width="120" class="mb-4 historical-form">
        ...
        <div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 mt-2">
          ...
          <div class="flex justify-end items-end">
            <n-button round type="primary" @click="queryData">查询数据</n-button>
            <n-button round @click="resetQuery" class="ml-2">重置</n-button>
          </div>
        </div>
      ...
      <n-tabs type="line" v-model:value="activeTab" @update:value="handleTabChange">
        <n-tab-pane name="trend" tab="趋势分析">
          <div class="historical-panel">
            <n-card title="参数趋势" size="small" class="mb-4 historical-sub-card">
            ...
            <div class="grid grid-cols-2 gap-4">
              <n-card title="温度分析" size="small" class="historical-sub-card">
                <div class="h-64">
                  <!-- 温度分析图表 -->
                  <div style="width: 100%; height: 100%" ref="temperatureChartRef"></div>
                </div>
              </n-card>
              <n-card title="压力分析" size="small" class="historical-sub-card">
                <div class="h-64">
                  <!-- 压力分析图表 -->
        ...
        <n-tab-pane name="statistics" tab="统计分析">
          <div class="historical-panel">
            ...
            <n-card title="参数分布" size="small" class="historical-sub-card">
        ...
        <n-tab-pane name="report" tab="报表生成">
          <div class="historical-panel">
            <n-card title="报表配置" size="small" class="mb-4 historical-sub-card">
              <n-form label-placement="left" label-width="120" class="historical-form">
                ...
                <n-form-item>
                  <n-button round type="primary" :loading="reportGenerating" :disabled="reportGenerating" @click="generateReport">
                    生成报表
                  </n-button>
                </n-form-item>
                ...
                <n-form-item v-if="lastReportInfo" label="最近生成结果">
                  <div class="w-full">
                    ...
                    <div class="mt-2 flex gap-2">
                      <n-button round size="small" type="primary" ghost @click="handleDownload(lastReportInfo)">下载报表</n-button>
                      <n-button round size="small" @click="copyStoragePath(lastReportInfo.storagePath)">复制存储路径</n-button>
                    </div>
                  </div>
                </n-form-item>
              </n-form>
            ...
            <n-card title="历史报表" size="small" class="historical-sub-card">
...
<script setup lang="ts">
  ...
  const startDate = ref(new Date('2026-01-01'));
  ...
  const queryData = async () => {
    ...
    try {
      ...
      const allDataResponse = await dataManagementApi.getDataList({
      ...
      const allData = Array.isArray(allDataResponse?.data)
        ...
        : Array.isArray(allDataResponse?.data?.data)
...
<style lang="less" scoped>
  ...
  .historical-form :deep(.n-base-selection),
  .historical-form :deep(.n-input),
  .historical-form :deep(.n-input-number) {
    border-radius: 10px;
  }
</style>
