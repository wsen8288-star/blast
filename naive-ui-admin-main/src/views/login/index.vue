        ...
        >
          ...
          <n-form ref="formRef" label-placement="top" size="large" :model="formInline" :rules="rules" require-mark-placement="right-hanging">
            ...
            <div class="flex items-center justify-between mb-8 mt-2">
              ...
              <router-link to="/forgot-password" class="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">
                忘记密码？
              </router-link>
            </div>
          ...
          <div class="mt-8 text-center text-sm text-neutral-400">
            还没有系统账号？
            <router-link to="/register" class="font-medium text-white hover:text-blue-400 transition-colors border-b border-transparent hover:border-blue-400 pb-0.5">
              立即注册
            </router-link>
          </div>
  ...
  const handleSubmit = (e) => {
    ...
    formRef.value.validate(async (errors) => {
      if (!errors) {
        ...
        try {
          const response: any = await userStore.login({ username, password, role });
