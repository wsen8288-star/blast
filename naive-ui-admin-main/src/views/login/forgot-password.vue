...
<script lang="ts" setup>
  ...
  const rules = { email: { required: true, message: '请输入邮箱', trigger: 'blur', type: 'email' } };
  ...
  const handleSubmit = async (e) => {
    ...
    formRef.value.validate(async (errors) => {
      if (!errors) {
        ...
        loading.value = true;
        try {
          const response: any = await request({ url: '/api/auth/forgot-password', method: 'post', data: { email } });
          message.destroyAll();
          if (response.code === ResultEnum.SUCCESS) {
            message.success(`临时密码已生成：${response.data}`);
            setTimeout(() => { router.push({ path: '/reset-password', query: { email: formInline.email } }); }, 3000);
          } else {
            message.error(response.msg || '生成临时密码失败');
          }
        } catch (error) {
          message.destroyAll();
