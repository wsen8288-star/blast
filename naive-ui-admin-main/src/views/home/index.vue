<template>
  <div class="home-portal min-h-[calc(100vh-80px)] p-4 lg:p-6">
    <div class="flex flex-col gap-6">
      ...
      >
        <div class="flex items-center justify-between gap-4 home-hero-content">
          <div class="min-w-0">
            <div class="home-hero-title">
              高炉生产参数优化系统
            </div>
            <div class="mt-1 text-sm home-hero-subtitle">
              数据采集、监控预警、分析建模与参数优化一体化
            </div>
          </div>
          <div class="flex gap-2 shrink-0">
            <n-button round type="primary" @click="go('/blast-furnace/data-management/data-collection')">
              去数据采集
            </n-button>
            <n-button round secondary @click="go('/blast-furnace/optimization/model-training')">去模型训练</n-button>
          </div>
        </div>
      </n-card>
      ...
      >
        ...
        <div class="mt-3 flex items-center gap-2">
          <n-button size="small" round type="primary" @click="continueQuickStart">继续本轮</n-button>
          <n-button size="small" round secondary @click="startNewQuickStart">开始新一轮</n-button>
        </div>
      </n-card>
...
<style scoped>
  .home-hero-card {
    ...
    border-radius: 14px;
    background: linear-gradient(120deg, color-mix(in srgb, var(--n-primary-color) 10%, var(--n-card-color)) 0%, var(--n-card-color) 65%);
