...
export function generatorMenu(routerMap: Array<any>) {
  ...
  filterRouter(routerMap).forEach((item) => {
    const isRoot = isRootRouter(item);
    const info = isRoot ? item.children[0] : item;

    // 检查是否是 BlastFurnace 路由，如果是，直接返回其子菜单
    if (item.name === 'BlastFurnace' && info.children && info.children.length > 0) {
      // 递归处理子菜单
      const childMenus = generatorMenu(info.children);
      result = result.concat(childMenus);
    } else {
      // 不是 BlastFurnace 路由，创建当前菜单
      const currentMenu = {
        ...info,
        ...info.meta,
        label: info.meta?.title,
        key: info.name,
        // 对于字符串格式的图标，使用 renderFontClassIcon 函数渲染
        icon: isRoot
          ? typeof item.meta?.icon === 'string'
            ? renderFontClassIcon(item.meta?.icon)
            : item.meta?.icon
          : typeof info.meta?.icon === 'string'
          ? renderFontClassIcon(info.meta?.icon)
          : info.meta?.icon,
      };
      // 是否有子菜单，并递归处理
