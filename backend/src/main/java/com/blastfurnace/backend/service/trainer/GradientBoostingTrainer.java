...
// 必须引入这个包才能使用具体的 Loss 函数
import smile.base.cart.Loss;

/**
 * 梯度提升树训练器 - 使用Smile库实现
 */
public class GradientBoostingTrainer implements ModelTrainer {
    ...
    public TrainingResult train(List<ProductionData> trainingData, ModelTraining modelTraining, 
        ...
        try {
            ...
            System.out.println("========== 开始Smile梯度提升树训练 ==========");
            ...
            if (numTrees < 1 || numTrees > 5000) {
                throw new TrainingException("梯度提升树的迭代次数不合法: " + numTrees);
            }
            
            int maxDepth = modelConfig.getMaxDepth() != null ? modelConfig.getMaxDepth() : 6; // 默认深度6
            if (maxDepth < 1 || maxDepth > 50) {
                throw new TrainingException("梯度提升树的最大深度不合法: " + maxDepth);
            }
            
            double shrinkage = modelConfig.getLearningRate() != null ? modelConfig.getLearningRate() : 0.1; // 优先使用GBDT专用学习率
            if (Double.isNaN(shrinkage) || Double.isInfinite(shrinkage) || shrinkage <= 0 || shrinkage > 1.0) {
                throw new TrainingException("梯度提升树的学习率不合法: " + shrinkage);
            }
            
            int maxNodes = modelConfig.getMaxNodes() != null ? modelConfig.getMaxNodes() : 64;
            int nodeSize = modelConfig.getNodeSize() != null ? modelConfig.getNodeSize() : 16;
            double subsample = modelConfig.getSubsample() != null ? modelConfig.getSubsample() : 0.8;

            if (maxNodes < 2 || maxNodes > 100000) {
                throw new TrainingException("梯度提升树的最大节点数不合法: " + maxNodes);
            }
            if (nodeSize < 1 || nodeSize > 100000) {
                throw new TrainingException("梯度提升树的叶子最小样本数不合法: " + nodeSize);
            }
            if (Double.isNaN(subsample) || Double.isInfinite(subsample) || subsample <= 0 || subsample > 1.0) {
                throw new TrainingException("梯度提升树的子采样率不合法: " + subsample);
            }
            int dataSize = trainList.size();
            int safeNodeSize = Math.min(nodeSize, Math.max(2, dataSize / 10));
            safeNodeSize = Math.min(safeNodeSize, Math.max(1, dataSize));
            
            System.out.println("梯度提升树参数 - 树数量: " + numTrees + ", 最大深度: " + maxDepth + ", 学习率: " + shrinkage + ", 采样率: " + subsample + ", 最大节点数: " + maxNodes + ", 叶子最小样本: " + safeNodeSize);
            
            if (Thread.currentThread().isInterrupted()) {
                throw new TrainingException("训练已取消", new InterruptedException("训练已取消"));
            }
            Formula formula = Formula.lhs("target");
            String lossConfig = modelConfig.getLossFunction();
            Loss lossFunction;
            if (lossConfig == null || lossConfig.isBlank() || "mse".equalsIgnoreCase(lossConfig)) {
                lossFunction = Loss.ls();
            } else if ("mae".equalsIgnoreCase(lossConfig)) {
                lossFunction = Loss.lad();
            } else if ("huber".equalsIgnoreCase(lossConfig)) {
                lossFunction = Loss.huber(1.0);
            } else {
                lossFunction = Loss.ls();
            }
            GradientTreeBoost model;
            TrainingRuntimeGuards.acquireTreeModelPermit();
            try {
                model = GradientTreeBoost.fit(formula, trainDf,
                        lossFunction, numTrees, maxDepth, maxNodes, safeNodeSize, shrinkage, subsample);
            } finally {
            ...
            throw new TrainingException("梯度提升树训练失败: " + e.getMessage(), e);
        }
    }
    ...
    public TrainingResult evaluate(List<ProductionData> evaluationData, ModelTraining modelTraining, 
        ...
        if (!(model instanceof GradientTreeBoost gbtModel)) {
        ...
        for (int i = 0; i < filteredEvaluationData.size(); i++) {
            actuals[i] = evalData[i][inputSize];
            predictions[i] = gbtModel.predict(evalDf.get(i));
        }
        ...
        String message = "梯度提升树评估完成";
        if (droppedTargets > 0) {
            message = message + " (丢弃目标缺失样本: " + droppedTargets + ")";
        }
        TrainingResult result = new TrainingResult(loss, r2Score, mae, rmse, true, message);
        result.setModel(gbtModel);
        result.setFeatures(selectedFeatures);
        ...
        try {
            double[] importance = gbtModel.importance();
            int impIdx = 0;
            for (String feature : selectedFeatures) {
                if (!targetVariable.equals(feature) && impIdx < importance.length) {
                    fiMap.put(feature, Math.max(0, importance[impIdx]));
                    impIdx++;
                }
            }
    ...
    public Object deserializeModel(byte[] modelBytes) throws Exception {
        ...
        try {
            Object model = deserializeWithSmile(modelBytes, GradientTreeBoost.class);
            if (model != null) {
                return model;
            }
        } catch (Exception ignored) {
