<template>
  <div
    class="data-preprocessing w-full p-4 md:p-6 box-border"
  >
    <n-card title="数据预处理" class="mb-4 border">
      <n-tabs ref="tabsRef" type="line" v-model:value="activeTab">
        <n-tab-pane name="import" tab="数据导入">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <div class="mb-4 flex items-center gap-2">
              ...
              />
              <n-button @click="loadImportTemplates">刷新模板</n-button>
              <n-button type="success" :disabled="!importPreviewId" @click="confirmImport">确认导入</n-button>
            </div>
            ...
            <n-card
              title="数据预览"
              size="small"
              class="mb-4 overflow-x-auto border"
            >
        ...
        <n-tab-pane name="process" tab="预处理设置">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <n-card title="预处理步骤" size="small" class="mb-4 border">
              <n-form label-placement="left" label-width="120">
                ...
                <n-form-item>
                  <n-button type="primary" @click="startPreprocessing">开始预处理</n-button>
                  <n-button @click="resetSettings" class="ml-2">重置设置</n-button>
                </n-form-item>
              </n-form>
            </n-card>

            <n-card title="预处理进度" size="small" v-if="isProcessing" class="border">
        ...
        <n-tab-pane name="result" tab="处理结果">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="flex items-center gap-2">
                <n-select
                  ...
                  style="width: 220px"
                />
                <n-button type="primary" @click="exportResult">导出处理结果</n-button>
                <n-button type="info" @click="exportSnapshot">导出快照包</n-button>
              </div>
            </div>
            ...
            <n-card title="处理后数据预览" size="small" class="overflow-x-auto border">
    ...
    try {
      ...
      if (res.code === 200) {
        ...
        if (processedDataList.length > 0) {
          ...
          // 为异常值添加标记并生成异常信息
          const markedData = processedDataList.slice(0, PREVIEW_LIMIT).map((row: any) => {
            const newRow = { ...row };
            ...
            if (row.is_outlier) {
              ...
              outlierColumns.forEach((col: string) => {
            ...
            if (row.is_out_of_range) {
              const outOfRangeColumns = row.out_of_range_columns
                ...
                : [];
              outOfRangeColumns.forEach((col: string) => {
                if (row[col] != null && row[col] !== '') {
