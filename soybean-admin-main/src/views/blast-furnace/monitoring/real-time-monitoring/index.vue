<template>
  <div class="rounded-2xl border p-2 transition-colors duration-300" :class="monitorShellClass">
    <div class="relative rounded-xl bg-slate-950 text-slate-100">
    <div
    ...
    ></div>
    <div class="relative z-10 w-full px-4 py-4 pb-8 lg:px-6 lg:py-5 lg:pb-10">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-300">
              <span class="rounded-md border border-slate-700/60 bg-slate-900/55 px-2 py-0.5">
                炉号 {{ selectedFurnaceLabel }}
              </span>
              <span class="rounded-md border border-slate-700/60 bg-slate-900/55 px-2 py-0.5">
                时间 {{ nowText }}
              </span>
              ...
              <span class="rounded-md border border-slate-700/60 bg-slate-900/55 px-2 py-0.5 text-emerald-200">
        ...
        <n-config-provider :theme="darkTheme">
          <div class="flex flex-wrap items-center gap-2 rounded-xl border border-slate-700/50 bg-slate-900/35 px-2 py-2">
            <div
            ...
            <n-button size="small" :type="preferLive ? 'primary' : 'default'" @click="switchToLive">
              实时数据
            </n-button>
            <n-button
              v-if="enableDemoMock"
              size="small"
              :type="preferLive ? 'default' : 'primary'"
              @click="switchToMock"
            >
              演示模拟
            </n-button>
            <n-button size="small" :type="effectiveStrictLive ? 'default' : 'warning'" @click="toggleStrictLive">
              {{ effectiveStrictLive ? '严格实时' : '宽松回滚' }}
            </n-button>
            <n-button v-if="enableDemoMock" size="small" quaternary @click="burstAnomaly">注入异常</n-button>
          </div>
        </n-config-provider>
      </div>
      <div v-if="preferLive && showFallbackOptions" class="mt-3 rounded-lg border border-amber-300/25 bg-amber-500/10 p-3">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          <n-button size="small" type="warning" @click="retryLive">立即重试</n-button>
          <n-button size="small" @click="keepFrozen">保持冻结</n-button>
          <n-button v-if="enableDemoMock" size="small" type="error" ghost @click="switchToMock">切换模拟</n-button>
        </div>
      </div>
      ...
      <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-4">
        <div class="relative overflow-hidden rounded-xl border border-slate-700/50 bg-slate-900/35 p-4">
          ...
          <div class="mt-2 flex items-end gap-3">
            <div class="font-mono text-4xl font-semibold tracking-tight lg:text-5xl" :class="kpiGlowClass">
        ...
        <div class="rounded-xl border border-slate-700/50 bg-slate-900/35 p-4">
          <div class="flex items-center justify-between">
            ...
            <div class="mt-3 flex items-end gap-2">
              <div class="font-mono text-4xl font-semibold tracking-tight lg:text-5xl" :class="healthGlowClass">
              {{ healthScore }}
            </div>
        ...
        <div class="col-span-12 lg:col-span-9">
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            >
              ...
              <div class="mt-2 h-[220px] w-full md:h-[240px] 2xl:h-[280px]" ref="windTempChartRef"></div>
            </div>
            <div
            ...
            >
              ...
              <div class="mt-2 h-[220px] w-full md:h-[240px] 2xl:h-[280px]" ref="windPressureChartRef"></div>
            </div>
            <div
            ...
            >
              ...
              <div class="mt-2 h-[220px] w-full md:h-[240px] 2xl:h-[280px]" ref="windVolumeChartRef"></div>
            </div>
            <div
            ...
            >
              ...
              <div class="mt-2 h-[220px] w-full md:h-[240px] 2xl:h-[280px]" ref="coalInjectionChartRef"></div>
            </div>
          </div>
        </div>
          ...
          >
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center justify-between gap-2">
                        ...
                        <div class="shrink-0 font-mono text-[10px] text-slate-300/80">{{ row.time }}</div>
                      </div>
                      <div class="mt-0.5 truncate text-xs text-slate-300/80">{{ row.detail }}</div>
                      <div class="mt-1 flex items-center gap-2">
                        <n-button size="tiny" quaternary @click.stop="gotoAnomaly(row)">
                          查看异常
                        </n-button>
                        <n-button
                          size="tiny"
                          type="primary"
                          quaternary
                          :disabled="!row.warningId"
                          @click.stop="gotoWarning(row)"
                        >
                          去预警处理
                        </n-button>
                      </div>
            ...
            <div class="mt-3 rounded-lg border border-slate-700/40 bg-slate-950/40 px-3 py-2">
              ...
              <div class="mt-2 space-y-1.5">
                ...
                <div class="max-h-24 space-y-1.5 overflow-y-auto pr-1">
                  <div v-for="item in sourceAuditList" :key="item.id" class="flex items-center gap-2 text-[11px]">
                    <span class="shrink-0 font-mono text-slate-400">{{ item.time }}</span>
                    <span class="min-w-0 flex-1 truncate text-slate-300">{{ formatSourceAuditEvent(item.event) }}</span>
                  </div>
                </div>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  const refreshHz = 1;
  const enableDemoMock = true;
  const furnaceIds = ['BF-001', 'BF-002', 'BF-003'] as const;
  const mixedFurnaceId = 'BF-MIX' as const;
  type FurnaceSelection = (typeof furnaceIds)[number] | typeof mixedFurnaceId;
  const furnaceOptions = [
    { label: '三炉混合', value: mixedFurnaceId },
    ...furnaceIds.map((id) => ({ label: id, value: id })),
  ];
  const selectedFurnace = ref<FurnaceSelection>('BF-001');
  const isMixedFurnace = computed(() => selectedFurnace.value === mixedFurnaceId);
  const selectedFurnaceLabel = computed(() => (isMixedFurnace.value ? '三炉混合' : selectedFurnace.value));
  const router = useRouter();
  const themeStore = useThemeStore();
  const monitorShellClass = computed(() =>
    themeStore.darkMode
  ...
  const lastSuccessText = computed(() => {
    ...
    return formatClock(new Date(lastSuccessAt.value));
  });
  ...
  const aggregateRawRows = (rows: any[]) => {
    const valid = rows.filter(Boolean);
    if (!valid.length) return null;
    const avgValue = (keys: string[]) => {
      const nums: number[] = valid
        .map((row) => keys.map((k) => Number(row?.[k])).find((v) => Number.isFinite(v)))
        .filter((v): v is number => typeof v === 'number' && Number.isFinite(v));
      if (!nums.length) return undefined;
      return nums.reduce((sum: number, n: number) => sum + n, 0) / nums.length;
    };
    const timestamps = valid
      .map((row) => row?.timestamp || row?.createTime || row?.time)
      .filter((v) => v !== undefined && v !== null);
    return {
      timestamp: timestamps[timestamps.length - 1],
      windTemperature: avgValue(['windTemperature', 'windTemp', 'furnaceTemperature', 'temperature']),
      windPressure: avgValue(['windPressure', 'pressure']),
      windVolume: avgValue(['windVolume', 'airFlow']),
      coalInjection: avgValue(['coalInjection', 'coalFlow']),
      hotMetalTemperature: avgValue(['hotMetalTemperature', 'hotMetalTemp']),
      siliconContent: avgValue(['siliconContent']),
    };
  };
  ...
  const pushSourceAudit = (event: string) => {
    const maxSourceAuditCount = 12;
    sourceAuditList.value.unshift({
      id: makeId(),
      time: formatClock(new Date()),
      event,
    });
    if (sourceAuditList.value.length > maxSourceAuditCount) sourceAuditList.value.splice(maxSourceAuditCount);
  };

  const sourceAuditTokenText: Record<string, string> = {
    REALTIME: '实时模式',
    MOCK: '演示模式',
    STRICT: '严格实时',
    LENIENT: '宽松回滚',
    ...
    FROZEN: '冻结显示',
    FLOWING: '正常流动',
    OVERRIDE_STRICT: '手动切换：严格实时',
    OVERRIDE_LENIENT: '手动切换：宽松回滚',
  };

  const formatSourceAuditEvent = (event: string) => {
    const text = String(event || '').trim();
    if (!text) return '-';
    if (!text.includes('|')) {
      return sourceAuditTokenText[text] || text;
    }
    return text
      .split('|')
      .map((token) => sourceAuditTokenText[token] || token)
      .join('｜');
  };
  ...
  const loadResolvedRanges = async () => {
    try {
      const payloads = isMixedFurnace.value
        ? await Promise.all(
            furnaceIds.map(async (furnaceId) => {
              const res: any = await anomalyConfigApi.getEffectiveThresholds({ furnaceId });
              return res?.data;
            })
          )
        : [((await anomalyConfigApi.getEffectiveThresholds({ furnaceId: selectedFurnace.value })) as any)?.data];
      const payload = payloads.find((item) => item && !Array.isArray(item));
      ...
      Object.keys(payload).forEach((key: string) => {
        const merged = payloads
          ...
          .filter(Boolean);
        const sourceItems = merged.length ? merged : [payload[key]];
        const avg = (field: string) => {
          const nums = sourceItems.map((item: any) => Number(item?.[field])).filter((v: number) => Number.isFinite(v));
          if (!nums.length) return NaN;
          return nums.reduce((sum: number, n: number) => sum + n, 0) / nums.length;
        };
        const min = avg('min');
        const max = avg('max');
        const warningMin = avg('warningMin');
        const warningMax = avg('warningMax');
        ...
        next[key] = {
          ...
          warningMax,
          label: paramLabels[key] || key,
          source: isMixedFurnace.value
            ? 'MIX'
            : String((sourceItems[0] as any)?.source || 'DEFAULT').toUpperCase(),
        };
      });
  ...
  const tickMock = () => {
    ...
    if (!isHotMetalOk.value) {
      pushAlarm(
        ...
        `铁水温度异常 · ${selectedFurnaceLabel.value}`,
    ...
    if (!isSiliconOk.value) {
      pushAlarm(
        ...
        `含硅量异常 · ${selectedFurnaceLabel.value}`,
    ...
    if (!isWindTempOk.value && Math.random() < 0.4) {
      pushAlarm(
        'info',
        `风温偏离 · ${selectedFurnaceLabel.value}`,
    ...
    if (!isWindPressureOk.value && Math.random() < 0.4) {
      pushAlarm(
        'info',
        `风压波动 · ${selectedFurnaceLabel.value}`,
  ...
  const syncLatestData = async () => {
    try {
      const wrappers = isMixedFurnace.value
        ? await Promise.all(
            furnaceIds.map(async (furnaceId) => {
        ...
        : [
            ((await dataManagementApi.getLatestData({
              furnaceId: selectedFurnace.value,
              strictLive: effectiveStrictLive.value,
            })) as any)?.data || {},
          ];
      const sourceValues = wrappers.map((wrapper) => String(wrapper?.source || 'LIVE').toUpperCase());
      latestSource.value = sourceValues.includes('LIVE')
        ? 'LIVE'
        : sourceValues.includes('NO_LIVE_DATA')
        ? 'NO_LIVE_DATA'
        : 'MOCK';
      latestStale.value = wrappers.some((wrapper) => Boolean(wrapper?.stale));
      syncSourceAudit();
      const merged = aggregateRawRows(wrappers.map((wrapper) => wrapper?.data || wrapper));
      if (!merged) {
      ...
      const label = formatClockFromAny(merged?.timestamp);
      applyLatestData(merged);
  ...
  const preloadHistoryData = async () => {
    try {
      let rows: any[] = [];
      if (isMixedFurnace.value) {
        const lists = (
          await Promise.all(
            furnaceIds.map(async (furnaceId) => {
              const res: any = await dataManagementApi.getRecentData({ furnaceId, limit: 60 });
        ...
        for (let i = 0; i < maxLen; i += 1) {
          const merged = aggregateRawRows(lists.map((list) => list[i]).filter(Boolean));
  ...
  const syncRealtimeAnomalies = async () => {
    try {
      const rows: any[] = isMixedFurnace.value
        ? (
            await Promise.all(
              furnaceIds.map(async (furnaceId) => {
      ...
      const mapped = rows
        .map((row) => {
          ...
          const furnaceId = row?.furnaceId || selectedFurnaceLabel.value;
          ...
          return {
            ...
            warningId: Number.isFinite(Number(row?.id)) ? Number(row.id) : null,
  ...
  const onWarningEvent = (event: Event) => {
    ...
    if (!isMixedFurnace.value && furnaceId && furnaceId !== selectedFurnace.value) {
      return;
    }
    scheduleAnomalySync();
  };
  ...
  const resetMock = () => {
    ...
    const bias = base === 'BF-001' ? 0 : base === 'BF-002' ? 18 : base === 'BF-003' ? -14 : 2;
    ...
    alarmQueue.value = [
      {
        ...
        level: 'info',
        title: `监控链路就绪 · ${selectedFurnaceLabel.value}`,
        detail: '大屏已进入实时渲染模式',
      },
      {
        id: makeId(),
        time: nowText.value || formatClock(new Date()),
        level: 'info',
        title: `异常检测联动 · ${selectedFurnaceLabel.value}`,
  ...
  const burstAnomaly = () => {
    ...
    pushAlarm('urgent', `人工注入异常 · ${selectedFurnaceLabel.value}`, '用于演示预警滚动列表与发光提示');
  };

  const gotoAnomaly = (row: AlarmRow) => {
    const query = isMixedFurnace.value
      ? { _hl: String(Date.now()) }
      : { furnaceId: selectedFurnace.value, _hl: String(Date.now()) };
    router.push({
  ...
  const gotoWarning = (row: AlarmRow) => {
    if (!row.warningId) return;
    router.push({
      path: '/blast-furnace/monitoring/early-warning',
      query: {
        highlightId: String(row.warningId),
        _hl: String(Date.now()),
      },
    });
  };
  ...
  const runtimeTick = async () => {
    ...
    try {
      ...
      if (!ok && liveFailCount.value >= liveFailThreshold.value) {
        ...
        pushAlarm('warning', `数据源断开 · ${selectedFurnaceLabel.value}`, '请手动选择重试、冻结或切换模拟');
        syncSourceAudit();
        return;
      }
      ...
      if (runtimeCycles % 6 === 0) {
        ...
        if (!anomalyOk && runtimeCycles % 6 === 0) {
          pushAlarm('info', `异常接口不可用 · ${selectedFurnaceLabel.value}`, '当前仅展示核心指标趋势');
        }
      }
  ...
  const keepFrozen = () => {
    showFallbackOptions.value = false;
    liveFrozen.value = true;
    pushAlarm('info', `数据保持冻结 · ${selectedFurnaceLabel.value}`, '当前保持最后一帧实时数据，等待人工重试');
    syncSourceAudit();
  };
