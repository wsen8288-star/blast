<template>
  <div
    class="anomaly-detection w-full p-4 md:p-6 box-border"
  >
    <n-card title="异常检测" class="mb-4 border">
      <!-- 检测配置 -->
      <n-form label-placement="left" label-width="120" class="mb-4">
        ...
        <div class="flex justify-end">
          ...
          <n-button @click="resetDetection" class="ml-2">重置</n-button>
          <n-button v-if="enableDemoMock" @click="testAnomaly" class="ml-2" type="error" ghost>注入单条异常</n-button>
          <n-button v-if="enableDemoMock" @click="injectBatchData" class="ml-2" type="warning" ghost
            >注入批量数据(测试统计)</n-button
          >
          <n-button @click="configModalRef?.open()" class="ml-2" type="info" ghost>检测配置</n-button>
        </div>
      ...
      <!-- 提示信息 -->
      <n-alert title="检测说明" type="info" class="mb-4 border" closable>
      ...
      <n-tabs type="line" default-value="realtime" @update:value="handleTabChange">
        <n-tab-pane name="realtime" tab="实时异常监控">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            ...
            <n-card title="实时异常列表" size="small" class="border">
              <n-data-table
        ...
        <n-tab-pane name="analysis" tab="异常分析">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <n-card title="异常类型分布" size="small" class="mb-4 border">
              <div class="h-80" ref="distributionChartRef"></div>
            </n-card>

            <n-card title="异常趋势" size="small" class="border">
        ...
        <n-tab-pane name="history" tab="异常历史">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <n-form label-placement="left" label-width="120" class="mb-4">
              ...
              <div class="flex justify-end">
                <n-button type="primary" @click="queryHistory" :loading="loadingHistory"
                  >查询历史</n-button
                >
              </div>
            </n-form>

            <n-card title="异常历史记录" size="small" class="border">
              <n-data-table
