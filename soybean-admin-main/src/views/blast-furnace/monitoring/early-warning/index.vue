<template>
  <div
    class="early-warning w-full p-4 md:p-6 box-border"
  >
    <n-card title="预警管理" class="mb-4 border">
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <n-card size="small" :bordered="false" class="border">
      ...
      <n-card title="预警列表" size="small" class="mt-4 border">
        <div class="mb-4 flex flex-wrap items-center gap-3">
          ...
          <n-select
            ...
            placeholder="级别"
          />
          <n-button size="small" type="primary" :loading="loading" @click="reloadAll">查询</n-button>
          <n-button size="small" @click="resetFilters">重置</n-button>
          <n-button size="small" type="warning" :loading="batchHandling" @click="handleBatch">
            一键处理(提示/警告)
          </n-button>
        </div>
    ...
    <n-modal v-model:show="handleModalVisible" :mask-closable="false">
      ...
      >
        <div class="space-y-3">
          <div class="rounded-lg border px-3 py-2 text-sm" style="background-color: var(--n-color); border-color: var(--n-border-color); color: var(--n-text-color-3);">
            {{ handleTitle }}
          </div>
          <n-select
        ...
        <div class="mt-4 flex justify-end gap-2">
          <n-button size="small" @click="handleModalVisible = false">取消</n-button>
          <n-button size="small" type="primary" :loading="handleSubmitting" @click="submitHandle">
            提交
          </n-button>
        </div>
      </n-card>
...
<script setup lang="ts">
  ...
  const columns: DataTableColumns<WarningRecord> = [
    ...
    },
    {
      title: '负责人',
      key: 'handlerUser',
      render: (row) => row.handler || (row.handlerUser == null ? '-' : String(row.handlerUser)),
    },
    {
      title: '处理时间',
      key: 'handleTime',
...
<style lang="less" scoped>
  ...
  @keyframes row-highlight-pulse {
    0% {
      box-shadow: inset 0 0 0 0 rgba(245, 158, 11, 0.55);
      background-color: rgba(245, 158, 11, 0.10);
    }
    50% {
      box-shadow: inset 0 0 0 999px rgba(245, 158, 11, 0.22);
  ...
  :deep(.n-data-table tbody tr.warning-highlight > td) {
    animation: row-highlight-pulse 0.7s ease-in-out 5;
  }
</style>
