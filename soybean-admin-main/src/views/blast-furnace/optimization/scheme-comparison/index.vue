<template>
  <div
    class="scheme-comparison w-full p-4 md:p-6 box-border"
  >
    <n-card title="方案比较" class="mb-4 border">
      <div class="flex items-center justify-between mb-4">
        ...
        <n-button type="primary" :loading="evolutionLoading" @click="handleStartEvolution">
          开始演化计算
        </n-button>
      </div>
      <!-- 方案选择 -->
      <n-form label-placement="left" label-width="120" class="mb-4">
        ...
        <div class="flex justify-end">
          <n-button type="primary" @click="startComparison">开始比较</n-button>
          <n-button @click="resetComparisonConfig" class="ml-2">重置</n-button>
        </div>
      </n-form>
      ...
      <n-tabs type="line" v-model:value="activeTab">
        <n-tab-pane name="comparison" tab="方案比较">
          <div class="p-4 relative rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <div
              v-if="adoptExecuting"
            ...
            <n-card title="方案对比结果" size="small" class="mb-4 border">
            ...
            <n-card title="方案参数对比" size="small" class="border">
            ...
            <div class="flex justify-end mt-4">
              <n-button type="primary" size="large" :disabled="!canAdopt" @click="openAdoptModal">
                采纳并执行优化方案
              </n-button>
            </div>
          </div>
        ...
        <n-tab-pane name="analysis" tab="结果分析">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <n-card title="方案分析报告" size="small" class="border">
              <div class="mb-4">
                <n-text strong>方案A：{{ getSchemeLabel(schemeA) }}</n-text>
                <n-text class="ml-4">综合评分：{{ schemeAScoreText }}</n-text>
                <n-text class="ml-4" depth="3">偏好评分：{{ schemeAFitnessText }}</n-text>
              </div>
              <div class="mb-4">
                <n-text strong>方案B：{{ getSchemeLabel(schemeB) }}</n-text>
                <n-text class="ml-4">综合评分：{{ schemeBScoreText }}</n-text>
                <n-text class="ml-4" depth="3">偏好评分：{{ schemeBFitnessText }}</n-text>
              </div>
              <div class="mb-4">
              ...
              <div class="mt-4 flex justify-end">
                <n-button type="primary" ghost @click="exportComparisonReport">
                  一键导出方案报告
                </n-button>
              </div>
            </n-card>
            <n-card title="约束条件可视化" size="small" class="mt-4 border">
              <n-data-table
            ...
            <n-card title="方案可行性原因" size="small" class="mt-4 border">
              <n-list bordered>
        ...
        <n-tab-pane name="history" tab="历史记录">
          <div class="p-4 rounded-lg border" style="background-color: var(--n-color); border-color: var(--n-border-color);">
            <div class="rounded-lg border p-4 mb-4" style="background-color: var(--n-color); border-color: var(--n-border-color);">
              <div class="flex flex-wrap items-center gap-3">
                ...
                <n-select
                  ...
                  class="w-40"
                />
                <n-button type="primary" ghost size="small" @click="queryComparisonHistory">
                  查询历史
                </n-button>
                <div v-if="selectedHistoryRowKeys.length" class="flex items-center gap-2 ml-auto">
                  <n-dropdown :options="batchOptions" @select="handleBatchSelect">
                    <n-button type="warning" ghost size="small">批量操作</n-button>
                  </n-dropdown>
                </div>
              </div>
            </div>
            ...
            <n-tabs v-model:value="historyTab" type="segment">
              <n-tab-pane name="evolution" tab="演化历史">
                <div class="grid grid-cols-1 gap-4">
                  ...
                  <n-card title="演化方案库" size="small" class="border">
                    <n-data-table
              ...
              <n-tab-pane name="comparison" tab="比较记录">
                <n-card title="对比操作日志" size="small" class="border">
                  <n-data-table
              ...
              <n-tab-pane name="execution" tab="执行记录">
                <n-card title="方案执行日志" size="small" class="border">
                  <n-data-table
    ...
    <n-modal v-model:show="detailVisible" preset="card" :style="{ width: 'min(960px, 92vw)' }">
      <n-card title="演化详情" size="small" :bordered="false" class="border">
        <n-descriptions :column="2" size="small" class="mb-4">
          ...
          <n-descriptions-item label="推荐方案">{{ detailMeta.schemeA }}</n-descriptions-item>
          <n-descriptions-item :label="detailSecondaryLabel">{{ detailMeta.schemeB }}</n-descriptions-item>
          <n-descriptions-item label="推荐偏好评分">{{ detailMeta.scoreA }}</n-descriptions-item>
          <n-descriptions-item :label="detailSecondaryScoreLabel">{{ detailMeta.scoreB }}</n-descriptions-item>
          <n-descriptions-item label="推荐结论">{{ detailMeta.result }}</n-descriptions-item>
        </n-descriptions>
        <n-data-table
        ...
        />
        <n-card title="寻优维度" size="small" class="mt-4 border">
          <n-data-table
    ...
    <n-modal v-model:show="compareDetailVisible" preset="card" :style="{ width: 'min(960px, 92vw)' }">
      <n-card title="比较详情" size="small" :bordered="false" class="border">
        <n-descriptions :column="2" size="small" class="mb-4">
          ...
          <n-descriptions-item label="综合评分A">{{ compareDetailMeta.scoreA }}</n-descriptions-item>
          <n-descriptions-item label="综合评分B">{{ compareDetailMeta.scoreB }}</n-descriptions-item>
          <n-descriptions-item label="综合结论">{{ compareDetailMeta.result }}</n-descriptions-item>
        </n-descriptions>
        <n-data-table
...
<script setup lang="ts">
  ...
  const selectedCompareParams = ref(['productionRate', 'energyConsumption', 'temperature', 'pressure', 'gasFlow']);
  ...
  const isBetterMetric = (param: string, diff: number) => {
    if (param === 'productionRate') {
      return diff > 0;
    }
    if (param === 'energyConsumption') {
      return diff < 0;
    }
  ...
  const constraintColumns: DataTableColumns<any> = [
  ...
  // 历史查询
  const historyDateRange = ref<[number, number]>([Date.now() - 30 * 24 * 60 * 60 * 1000, Date.now()]);
  ...
  const selectedHistoryRowKeys = ref<DataTableRowKey[]>([]);
  ...
  const baseEvolutionParamOptions = [
    { label: '偏好评分', value: 'fitness' },
    { label: '炉温 (℃)', value: 'temperature' },
    { label: '炉顶压力 (kPa)', value: 'pressure' },
  ...
  const fetchExecutionLogs = async () => {
    try {
      const res: any = await comparisonApi.getOperationLogs({ page: 0, size: 200 });
      const data = res?.data;
  ...
  const compareHistoryColumns: DataTableColumns<any> = [
    { type: 'selection', width: 50, align: 'center' },
  ...
  const executionLogColumns: DataTableColumns<any> = [
    { title: '记录ID', key: 'id', width: 80, align: 'center' },
    ...
    { title: '操作人', key: 'operator', width: 100, align: 'center' },
    {
      ...
      render: (row: any) => {
        try {
          ...
          if (data.pressure) parts.push(`压力:${formatNumber(data.pressure)}kPa`);
  ...
  const detailSecondaryIsBaseline = ref(false);
  const detailSecondaryLabel = computed(() => (detailSecondaryIsBaseline.value ? '基准工况' : '备选方案'));
  const detailSecondaryScoreLabel = computed(() =>
    detailSecondaryIsBaseline.value ? '基准偏好评分' : '备选偏好评分'
  );
  ...
  const detailColumns: DataTableColumns<any> = [
    ...
    { title: '预测模型', key: 'modelUsed' },
    {
      ...
      render: (row: any) => `${formatNumber(row.predictedProduction)} t/h`,
    },
    {
      title: '能耗',
      key: 'estimatedEnergy',
    ...
    },
    {
      title: '压力',
      key: 'pressure',
      render: (row: any) => `${formatNumber(row.pressure)} kPa`,
    },
    {
      title: '煤气流量',
      key: 'gasFlow',
    ...
    { title: '偏好评分', key: 'fitness', render: (row: any) => formatScoreLabel(row.fitness) },
    { title: '评分解释', key: 'explanation' },
  ];
  const detailGeneColumns: DataTableColumns<any> = [
  ...
  const compareDetailColumns: DataTableColumns<any> = [
  ...
  const startComparison = async () => {
    ...
    try {
      ...
      const handleResult = async (res: any) => {
        ...
        schemeAFitness.value =
          typeof result.fitnessA === 'number'
            ? Math.round(result.fitnessA * 100) / 100
            : null;
        schemeBFitness.value =
          typeof result.fitnessB === 'number'
            ? Math.round(result.fitnessB * 100) / 100
            : null;
        ...
        if (historyDateRange.value && historyDateRange.value.length === 2) {
          historyDateRange.value = [historyDateRange.value[0], Date.now()];
        }
  ...
  const resetComparisonConfig = () => {
    ...
    selectedCompareParams.value = ['productionRate', 'energyConsumption', 'temperature', 'pressure', 'gasFlow'];
  ...
  const waitEvolutionResult = async (taskId: string) => {
    ...
    while (Date.now() - startAt < timeoutMs) {
      const progressRes: any = await optimizationApi.getOptimizationProgress(taskId);
      ...
      if (status === 'completed') {
        const resultRes: any = await optimizationApi.getOptimizationResult(taskId);
  ...
  const handleStartEvolution = async () => {
    ...
    try {
      ...
      const res: any = await optimizationApi.startEvolutionaryOptimization({
        runId: resolveRunId(),
  ...
  const fetchRunningServices = async () => {
    ...
    try {
      const res: any = await optimizationApi.deployment.getRunningServices();
  ...
  const updateEvolutionProcessFromHistory = async (historyId: number) => {
    try {
      const res: any = await comparisonApi.getEvolutionHistoryDetail(historyId);
  ...
  const fetchEvolutionHistory = async () => {
    try {
      ...
      const res: any = await comparisonApi.getEvolutionHistory(params);
      const rows = (res?.data || []) as Array<any>;
  ...
  const fetchCompareHistory = async () => {
    try {
      ...
      const res: any = await comparisonApi.getCompareHistory(params);
      const rows = (res?.data || []) as Array<any>;
  ...
  const handleViewHistory = async (row: any) => {
    ...
    try {
      const res: any = await comparisonApi.getEvolutionHistoryDetail(row.id);
      const detail = res?.data || {};
      detailSecondaryIsBaseline.value = String(detail.schemeB || '').includes('基准工况');
      detailMeta.value = {
        ...
        scoreA: formatScoreLabel(detail.scoreA),
        scoreB: formatScoreLabel(detail.scoreB),
      };
      let payload: any = {};
      if (detail.payload) {
        try {
      ...
      detailSolutions.value = solutions.slice(0, 2).map((solution: any, index: number) => ({
        ...solution,
        name:
          index === 0
            ? '智能优化方案'
            : detailSecondaryIsBaseline.value
              ? '基准工况'
              : '备选方案',
      }));
  ...
  const handleViewCompareHistory = async (row: any) => {
    ...
    try {
      const res: any = await comparisonApi.getCompareHistoryDetail(row.id);
      const detail = res?.data || {};
  ...
  const handleBatchDelete = () => {
    ...
    dialog.warning({
      ...
      onPositiveClick: async () => {
        try {
          const ids = selectedHistoryRowKeys.value
            .map((key) => Number(key))
            .filter((id) => Number.isFinite(id));
          await comparisonApi.batchDeleteHistory(ids);
  ...
  const schemeAFitnessText = computed(() => {
    ...
    return `${formatNumber(schemeAFitness.value)}/100`;
  });
  const schemeBFitnessText = computed(() => {
    if (schemeBFitness.value === null) {
      return '-';
    }
    return `${formatNumber(schemeBFitness.value)}/100`;
  });
  const scoreTypeText = computed(() => {
    if (scoreType.value === 'USER_SCORE') {
      return '综合评分用于对比展示；NSGA-II 搜索依据 rank/crowding，偏好评分用于推荐选择';
    }
  ...
  const feasibilityReasons = computed(() => {
    ...
    if (scoreType.value === 'USER_SCORE') {
      reasons.push('说明：NSGA-II 搜索依据 rank/crowding；偏好评分用于推荐选择；综合评分用于对比展示。');
    }
    return reasons;
  });
  ...
  const exportComparisonReport = () => {
    ...
    lines.push(`- 综合评分：A=${schemeAScoreText.value}，B=${schemeBScoreText.value}`);
    lines.push(`- 偏好评分：A=${schemeAFitnessText.value}，B=${schemeBFitnessText.value}`);
    lines.push(`- 推荐方案：${recommendedSchemeDisplay.value}`);
    lines.push(`- 综合评分口径：${scoreTypeText.value}`);
    lines.push('');
    lines.push('## 分析结论');
  ...
  const analysisConclusion = computed(() => {
    ...
    const productionRow = getRow('productionRate');
    const energyRow = getRow('energyConsumption');
  ...
  const fetchBaselineData = async () => {
    ...
    try {
      const res: any = await dataManagementApi.getLatestData({ furnaceId: selectedFurnace.value });
  ...
  const getDynamicRange = (param: string) => {
    ...
    const fallbackValue = Number(snapshot[param]);
  ...
  const getRadarValue = (row: any, key: 'schemeA' | 'schemeB', param: string) => {
    ...
    const higherBetter = param !== 'energyConsumption' && param !== 'cost';
  ...
  const buildRadarScores = () => {
    const production = getMetricRow('productionRate');
    const energy = getMetricRow('energyConsumption');
    ...
    return {
      schemeA: [
        getRadarValue(production, 'schemeA', 'productionRate'),
        getRadarValue(energy, 'schemeA', 'energyConsumption'),
      ...
      schemeB: [
        getRadarValue(production, 'schemeB', 'productionRate'),
        getRadarValue(energy, 'schemeB', 'energyConsumption'),
  ...
  const updateEvolutionChart = () => {
    ...
    setEvolutionOptions({
      ...
      legend: { data: ['最高偏好评分', '平均偏好评分'], show: true },
      grid: { left: 50, right: 30, top: 40, bottom: 30 },
      xAxis: {
        type: 'category',
        name: '代数',
