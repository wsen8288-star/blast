<template>
  <div class="w-full p-4 md:p-6 box-border space-y-4">
    <n-card :bordered="false" size="small" class="border shadow-sm">
      ...
      <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
        <n-card size="small" :bordered="false" class="border shadow-sm">
    ...
    <n-card :bordered="false" size="small" class="border shadow-sm">
      <div class="flex flex-wrap items-center gap-3">
        ...
        />
        <div class="flex items-center gap-2">
          <n-button size="small" type="primary" :loading="loading" @click="searchUser">查询</n-button>
      ...
      <div class="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2 text-sm transition-colors duration-300" style="color: var(--n-text-color-3);">
          ...
          <n-button v-if="checkedRowKeys.length" size="small" quaternary @click="clearSelection">清空选择</n-button>
        </div>
        <div class="flex items-center gap-2">
          <n-dropdown
    ...
    <n-drawer v-model:show="showCreateDrawer" :width="460" placement="right">
      <n-drawer-content title="新增用户" closable>
        <n-alert type="info" :bordered="false" class="border">
      ...
      <n-drawer-content title="编辑用户" closable>
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            ...
            <n-button type="primary" :loading="editing" @click="submitEdit">保存</n-button>
          </div>
        </template>
      </n-drawer-content>
    </n-drawer>
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          ...
          <n-button secondary @click="fillDefaultPassword">默认 123456</n-button>
          <n-button type="primary" :loading="resetting" @click="submitResetPassword">确认</n-button>
        </div>
      </template>
    </n-modal>
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <n-button @click="showBatchResetPasswordModal = false">取消</n-button>
          <n-button secondary @click="fillBatchDefaultPassword">默认 123456</n-button>
          <n-button type="primary" :loading="batchResetting" @click="submitBatchResetPassword">确认</n-button>
        </div>
      </template>
    </n-modal>
  </div>
...
<script setup lang="ts">
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const res: any = await systemApi.user.getList({
  ...
  const fetchRoles = async () => {
    try {
      ...
      roleOptions.value = roles
        .filter((r: any) => r && r.roleCode && r.roleName)
        .map((r: any) => ({ label: String(r.roleName), value: String(r.roleCode) }));
    } catch (e: any) {
      roleOptions.value = [];
    }
  };
  ...
  const updateUserStatus = async (row: UserRow) => {
    try {
      ...
      message.error(backendMsg || e?.msg || e?.message || '更新用户状态失败');
    }
  };
