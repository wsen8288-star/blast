  ...
  <div class="w-full p-4 md:p-6 box-border space-y-4">
    <n-card :bordered="false" size="small" class="border shadow-sm">
      <div class="flex items-start justify-between">
        <div>
          <div class="text-base font-semibold transition-colors duration-300" style="color: var(--n-text-color-1);">系统设置</div>
          <div class="mt-1 text-xs transition-colors duration-300" style="color: var(--n-text-color-3);">配置将实时联动算法、告警与数据清理模块</div>
        </div>
        <div class="flex items-center gap-2">
          ...
          <n-button type="primary" :loading="saving" @click="saveActive">
            <template #icon>
...
<script setup lang="ts">
  ...
  const forceNumberMinByKey = new Map<string, number>([
  ...
  const hiddenSystemConfigKeys = new Set<string>(['system_default_furnace_id']);
  ...
  async function loadGroup(group: SysConfigGroup) {
    ...
    try {
      ...
      const items: SysConfigEntity[] = (res.data || []).filter((item: SysConfigEntity) =>
        !(group === 'SYSTEM_CONFIG' && hiddenSystemConfigKeys.has(item.configKey))
      );
  ...
  async function saveGroup(group: SysConfigGroup) {
    ...
    try {
      ...
      if (group === 'SYSTEM_CONFIG') {
        await systemConfigStore.loadSystemConfig(true);
      }
    } catch (e: any) {
    ...
    setup(props, { emit }) {
      ...
      function renderControl(item: SysConfigEntity) {
        ...
        if (forceNumberConfigKeys.has(item.configKey)) {
          ...
          const minValue = forceNumberMinByKey.get(item.configKey) ?? 0;
          return h(NInputNumber, {
            value: Number.isFinite(current) ? current : 0,
            min: minValue,
            'onUpdate:value': (val: number | null) => emit('update', item.configKey, val ?? minValue),
            style: { width: '100%' },
            size: 'small',
          });
        }
